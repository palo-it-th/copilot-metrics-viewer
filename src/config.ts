const PROPS = ["MOCKED_DATA", "SCOPE", "GITHUB_ORG", "GITHUB_ENT", "GITHUB_TEAM", "GITHUB_TOKEN",
  "MSAL_CLIENT_ID", "MSAL_TENANT_ID", "MSAL_REDIRECT_URI"];
...
const config: Config = {
  ...
  msal: {
    ...
    tenantId: env.VUE_APP_MSAL_TENANT_ID,
    redirectUri: env.VUE_APP_MSAL_REDIRECT_URI,
  }
}
if (!config.mockedData && !config.github.token) {
  throw new Error("VUE_APP_GITHUB_TOKEN environment variable must be set.");
}
if (!config.mockedData && (!config.msal.clientId || !config.msal.tenantId || !config.msal.redirectUri)) {
  throw new Error("VUE_APP_MSAL_CLIENT_ID/VUE_APP_MSAL_TENANT_ID/VUE_APP_MSAL_REDIRECT_URI environment variable must be set.");
}
